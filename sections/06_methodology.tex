...
Many of the images are taken from the same view point, but with the focus point set to different grains.
The ground truth labeled are drawn for all present pollen grains, regardless of the how blurred they appear.
This is done so that the dataset may be modified for the purpose of our analysis of sharpness and model performance in regards to \textit{RQ2}.
...
This is demonstrated in Figure~\ref{fig:aspect}, where we can see that the grains are mostly square in shape and between 100 and 150 pixels wide.
...
We can clearly see that the grains are quite regular in both shape and size}\label{fig:aspect}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.2\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figs/method/fourier/square_fourier.png}
  \end{subfigure}%
  \hspace*{0.02\textwidth}
  \begin{subfigure}[t]{0.2\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figs/method/fourier/square_45deg_fourier.png}
  \end{subfigure}%
  \hspace*{0.02\textwidth}
  \begin{subfigure}[t]{0.2\textwidth}
    \centering
...
We ignore the phase component of the complex valued output, leaving \(\mathbf{X}\) to contain only the magnitude of the frequency components.
...
The main reason for this being its superior performance over VGG, and the ease with which it integrates with the auxiliary structure.
...
The figure is inspired by the one given in the original paper, but has been modified to reflect our alterations.
...
\begin{wrapfigure}[9]{r}{0.35\textwidth}
  \centering
  \includegraphics[width=0.35\textwidth]{figs/method/gausian_penelty.pdf}
...
With the goal of developing a system for automated pollen counting, we aim to explore how the peculiarities of this domain interact with the more established methods that have been developed for object detection.
For each experiment we will explain its rational and setup.
...
In the first experiment we will train the model with three different pre-trained feature extraction networks and compare them.
...
With pollen grains however, we have very uniform sizes and shapes throughout the dataset.
We believe that some or most of the feature maps can be removed from the model without any performance degradation.
To test this the model will be trained in multiple configurations where we remove certain feature maps from the model.
In practice, we will only deactivate the detection blocks that are fed by the feature maps so that we can deactivate layers arbitrarily without disturbing successive layers of the model.

\subsection{Minimum training sharpness}
As we have observed, the dataset contains a wide variety of grains spanning many levels of blur.
It remains unclear what impact blurry data has on the training of fully convolutional object detection systems.
With this experiment we aim to explore if there is a correlation between the distribution of sharpness in the training data and model predictions.
We are less interested in the performance of the model, and more interested in how the distribution of sharpness values in the training data changes the distribution of sharpness in both the predictions of the model and in the ground truths it does or does not detect.

For our experiment we will train the model on various versions of the same dataset.
...
We expect to see fewer correct predictions among the ground truths of the testing split which have a lower sharpness score that the threshold. 

\subsection{Cross-sharpness inference}
In the final experiment we wish to further explore what ability the model has to generalize outside of the sharpness values it is exposed to. 
We aim to uncover if a trained model can be transferred to different sharpness ranges.
Similar to the previous experiment we will train model on a subset of ground truths, however we will now also divide the testing set into to partitions.
With a baseline set containing all ground truths we then have three partitions of both training and testing data.
Each of the three resulting models will be tested on each of the tree test partitions, creating a kind on sharpness confusion matrix.
