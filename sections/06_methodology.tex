...
\textcolor{red}{Write about performace both FPS and training time}
...
The final dataset is then divided into a training and test set with a 85\% / 15\% ratio. 
...
\end{enumerate}
%
All the augmentations except cropping, independently, has a 50\% chance of being applied to each sample. Random crop also resizes the the sample input into a \(300\by 300\) square so it is needed for every image. The cropping algorithm also ensures that each sample contains at least one pollen grain.
