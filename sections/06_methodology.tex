...
A common filtering technique is called \emph{Non-maximum Suppression} (NMS).
Let \( \mathcal{B}=\{b_1,\ldots,b_N \} \) be a list of bounding boxes and \( \mathcal{S}=\{s_1,\ldots,s_N \} \) be their corresponding scores.
NMS is then a simple iterative filtering algorithm where, for each iteration, the bounding box with the highest score is selected from \(\mathcal{B}\), any bounding boxes that overlaps with the selected box is then pruned from \(\mathcal{B}\) and \(\mathcal{S}\) and the loop is continued until \(\mathcal{B}\) is exhausted. Boxes are considered overlapping if their IoU is larger than some threshold \(N_t\).
...
Given that the data in this project exhibits this characteristic we have replaced the standard NMS algorithm with a more modern version, Soft-NMS, introduced in~\cite{bodla2017softnms}. The pseudocode for both algorithms follows,
\linebreak
\begin{pseudofunc}{NMS}{\(\mathcal{B},~\mathcal{S},~N_t\)}
  ...
  \begin{pseudoloop}
    ...
    \begin{pseudoloop}
      ...
      \minipage[t]{\dimexpr0.48\linewidth-2\fboxsep-\fboxrule\relax}
...
\begin{enumerate}
  \item start with language and framework choice, short
  \item justify default-boxes, i.e.~keeping aspect unchanged, and adjusting sizes and removing the last tree layers because no boxes are matched to them
\end{enumerate}

\subsection{Experimental Setup}
\begin{enumerate}
  \item Hardware stack rundown, short
  \item pretend to care about hyper-parameters, we need not justify too much. We let it be implied.
  \item itemize the experiments
\end{enumerate}