...
Figure~\ref{fig:method-sharpness-mean} breaks down the ground truth labels into true positives and false negatives and we see that the model is failing to predict pollen grains which have a sharpness below the minimum sharpness of the training data.
It is important to note that the number of false negatives in each run is quite low, so the strength of any argument is weak.
It could also be that the data is showing that the model is overfitting and is therefore not generalizing to the more blurry examples.

Looking at the sharpness of the predictions made by the model shows a similar trend.
In Figure~\ref{fig:method-sharpness-detections} we observe that restricting the training data creates what seems to be a threshold, bellow which the model does not make predictions.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/method/sharpness/sharpness_TP_vs_FN.pdf}
  \caption[Sharpness distribution of ground truths in test split by minimum training sharpness]{%
Five experimental runs are shown, ground truth labels are filtered out of the training set by the sharpness requirement given in the plot header. Each plot gives the sharpness value for each GT in the test split divided into TP and FN\@.
  }\label{fig:method-sharpness-mean}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/method/sharpness/detection_sharpness_TP_vs_FP.pdf}
  \caption[Sharpness distribution of model predictions in test split by minimum training sharpness]{%
Explanation given in Figure~\ref{fig:method-sharpness-mean}, however sharpness is measured on predicted bounding boxes and is conversely divided into true positives and false positives.
  }\label{fig:method-sharpness-detections}
\end{figure}