...
With pollen we again have a much more uniform dataset where size differences between classes is negligible, which in turn might also explain why we don't observe any strong relationship between layer activation and performance.
This is compounded by the limited number of classes and could change if we were to use a larger, more diverse, dataset.
...
Figure~\ref{fig:results-sharp-gt} shows an example of the split, and we can see that all in focus objects are labeled ground truths. 
...
The boundary value set to the median sharpness value over the total training split.
...
The optimal sharpness distribution remains unknown although we have shown evidence to support that excluding blurry data causes the model too fixate on only the sharp features in images.