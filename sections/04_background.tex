...
We will start by looking at the methodology that is used currently for pollen counting and formalise it as a machine learning problem.
We will then provide a theoretical overview of the main building blocks of modern convolutional neural networks.
A basic understanding of the operation and conponents of a standard feed forward fully connected neural network is assumed for this section.
...
The patch of \(x\) involved in the sum at each step is refered to as the \textit{receptive field}.
As the operation is repeated for every index of \(x\), the receptive field slides accross the input.
The resulting output of the convolution is refered to as a \textit{feature map}.
...
The elementwise multiplication between the filter and receptive field and subsequent summation produces a single scaler in the feature map.
The operation is repeated over the index set of the input, producing the complete feature map.}\label{fig:cnn}
\end{figure}

Usually, the input to a convolutional layer contains multiple channels, e.g.\ an RGB input image which has three channels representing the seperate  red, green, and blue color channels.
To handle this we construct as many kernels as there are channels.
We then convolve each channel with its own kernel and add together the result accros the channels, which produces a single feature map.
An example of such a convolution operation is shown in Figure~\ref{fig:cnn}.
This stack of kernels is refered to as a \textit{filter}.
...
Dimensionality reduction is also possible using the concept of \textit{stride}, which referes to how the receptive field moves accross the input relative to the index of the feature map.
...
As such, further explenation of their action falls into the category of assumed prior knowledge.
...
As the distribution from previous layers change, this shift is propageated though to the layers downstream, and so each layer must deal with ever changing input distributions.
...
Given prediction \(\hat{X}\) with label \(\hat{X}_l\) and bounding box \(\hat{X}_u\). \(\hat{X}\) is considered a True Povitive if there exists a ground truth \(Y\), where \(Y_{l}=\hat{X}_l\) and \(\text{IoU}(\hat{X}_u, Y_{u})\ge \mu \).
...
%Training a Object detection models follows the familiar scheme of \textit{mini-batch gradient decent} using \textit{backpropagation} to determine the gradient of each parameter.  
