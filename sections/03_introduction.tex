...
The remainder of this documant is structured as follows: Section~\ref{cha:background} covers background knowlege relating to pollen imaging techniques, the composition and functioning of Convolutional Neural Networks, and metrics for meacuring performance in object detection tasks.
Section~\ref{cha:related} examins various object detection frameworks, as well as the existing work relating to pollen.
Section~\ref{cha:method} sketches the proposed system, while Section~\ref{cha:tech} explains its implementation and current results.
Finally, Section~\ref{cha:plans} layes out the plans for the next stage of the project.